  ...
  <div id="modal" class="items-center fixed left-0 top-0 z-1 px-8 w-full h-full bg-opacity-60 bg-black <?php if (isset($_SESSION['incorrect_info']) && $_SESSION['incorrect_info'] == 0) : ?> flex <?php else : ?> hidden <?php endif; ?>">
    <div class="flex flex-col gap-4 container mx-auto max-w-2xl h-auto p-5 shadow-xl bg-[#8fd396] rounded-2xl">
      <div class="w-full flex justify-between items-center">
        <h3 class="text-center text-base sm:text-xl text-black z-30">حروف را به درستی وارد کردید.</h3>
    ...
    <div class="shadow-xl bg-[#4F6F52] w-full max-w-lg h-auto p-7  rounded-2xl">
      <form action="postedData.php" method="POST" class="flex flex-col gap-3">
        <div class="flex flex-col gap-2">
          <label class="text-sm text-white">حروف درون کادر زیر را به ترتیب وارد کنید.</label>
          <div class="flex gap-2">
            <input name="letters" type="text" placeholder="برای مثال: ق ف م ل" class="w-full text-sm focus:shadow-lg focus:shadow-[#f0fbea]-500/40 border-2 focus:outline-none p-1 sm:p-1.5 rounded-xl 
            ...
            ?>
            <img class="m-auto w-17 h-7 sm:w-auto sm:h-auto" id="char_captcha" src="data:image/jpeg;base64,<?php $charactersCaptcha->show(); ?>" alt="captcha">
            <?php
            $charactersCaptcha->generateId(3);
            ?>

            <div class="m-auto hover:cursor-pointer" onclick="refreshCaptcha()">
          ...
          <?php if (isset($_SESSION['incorrect_info']) && $_SESSION['incorrect_info'] == 1) : ?> <span class="text-sm text-red-400 -mt-2">حروف را به ترتیب از راست به چپ وارد کنید.</span> <?php endif; ?>
...
<script>
  ...
  function refreshCaptcha() {
    fetch("./captcha.php?id=3")
      ...
      .then(data => {
        const captchaImage = document.querySelector('#char_captcha');
