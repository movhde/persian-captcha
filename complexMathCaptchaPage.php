  ...
  <div id="modal" class="items-center fixed left-0 top-0 z-1 px-8 w-full h-full bg-opacity-60 bg-black <?php if (isset($_SESSION['incorrect_info']) && $_SESSION['incorrect_info'] == 0) : ?> flex <?php else : ?> hidden <?php endif; ?>">
    <div class="flex flex-col gap-4 container mx-auto max-w-2xl h-auto p-5 shadow-xl bg-[#8fd396] rounded-2xl">
      <div class="w-full flex justify-between items-center">
        <h3 class="text-center text-base sm:text-xl text-black z-30">نتیجه را به درستی وارد کردید.</h3>
      ...
      <form action="postedData.php" method="POST">
        <div class="flex flex-col gap-2">
          ...
          <div class="flex gap-2">
            <input name="complexSum" type="text" placeholder="برای مثال :‌ ۲۱" class="w-full text-sm focus:shadow-lg focus:shadow-[#f0fbea]-500/40 border-2 focus:outline-none p-1 sm:p-1.5 rounded-xl 
            ...
            ?>
            <img class="m-auto w-17 h-7 sm:w-auto sm:h-auto" id="complex_captcha" src="data:image/jpeg;base64,<?php $complicatedMathCap->show(); ?>" alt="captcha">
            <?php
            $complicatedMathCap->generateId(2);
            ?>
...
<script>
  ...
  function refreshCaptcha() {
    fetch("./captcha.php?id=2")
      ...
      .then(data => {
        const captchaImage = document.querySelector('#complex_captcha');
